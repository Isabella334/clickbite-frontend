...
import { users as usersApi, helpers } from "../services/api";
...
const mockDeliveryPoints = [
  ...
  { id: 2, name: "Torre Empresarial",     zone: "Zone 10", address: "Blvd. Los Proceres 24-69, Zona 10",  icon: "🏢", lat: 14.5981, lng: -90.5133 },
  { id: 3, name: "Plaza Fontabella",      zone: "Zone 10", address: "Av. La Reforma 15-54, Zona 10",      icon: "🛍️", lat: 14.6016, lng: -90.5069 },
  { id: 4, name: "Parque Las Americas",   zone: "Zone 13", address: "Av. Las Americas, Zona 13",          icon: "🌳", lat: 14.5918, lng: -90.5234 },
  { id: 5, name: "Centro Historico",      zone: "Zone 1",  address: "6a Av. & 6a Calle, Zona 1",          icon: "🏛️", lat: 14.6407, lng: -90.5133 },
...
export default function OrderConfirm() {
  ...
  const handlePlace = async () => {
    ...
    setPlacing(true);
    try {
      // PATCH /users/:id/delivery-address - guarda las coordenadas del punto elegido
      if (session?.id && selectedPoint.lng != null && selectedPoint.lat != null) {
        await usersApi.updateDeliveryAddress(session.id, selectedPoint.lng, selectedPoint.lat);
      }
      // TODO: POST /orders once cart is wired to real state
      setPlaced(true);
    } catch (err) {
      // Delivery address patch failed - still let the order through, just log it
      console.warn("delivery-address patch failed:", err.message);
      setPlaced(true);
    } finally {
      setPlacing(false);
    }
