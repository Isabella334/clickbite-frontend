...
const CSS_DASHBOARD = `
  ...
  .rd-restaurant-card{display:flex;align-items:center;gap:10px;padding:12px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.06);border-radius:12px}
  ...
  .rd-popular{display:inline-flex;align-items:center;gap:4px;padding:3px 9px;border-radius:999px;background:rgba(251,191,36,0.1);border:1px solid rgba(251,191,36,0.2);color:#fbbf24;font-size:0.7rem;font-weight:600}
  ...
  .rd-delete-modal{background:#111820;border:1px solid rgba(224,92,92,0.25);border-radius:18px;width:100%;max-width:380px;padding:32px;text-align:center;animation:popIn 0.25s cubic-bezier(0.34,1.56,0.64,1)}
...
export default function RestaurantDashboard() {
  ...
  const [obRestForm,  setObRestForm]  = useState({
    name: "", description: "", category: "Otros", phone: "", email: session?.email ?? "",
  });
    ...
    const load = async () => {
      ...
      try {
        ...
        try {
          ...
          // 404 = este usuario aun no tiene restaurante → mostrar wizard
          if (e.status === 404 || (e.message && e.message.toLowerCase().includes("no se encontraron"))) {
            setOnboarding(true);
            setLoading(false);
            return;
          }
          throw e; // otro error real → propagar
        }
        ...
        setBestsellers((topItems ?? []).slice(0, 5).map(t => ({
          id: t.menu_item_id ?? t._id,
          name: t.name ?? t.item_name ?? "—",
          image: "🍽️",
          totalSold: t.total_sold ?? t.totalSold ?? 0,
        })));
      } catch (err) {
  ...
  const obHandleRestChange = (field, val) => {
    setObRestForm(p => ({ ...p, [field]: val }));
    if (obErrors[field]) setObErrors(p => { const n = { ...p }; delete n[field]; return n; });
  };

  const obHandleItemChange = (field, val) => {
    setObItemForm(p => ({ ...p, [field]: val }));
    const key = "item_" + field;
    if (obErrors[key]) setObErrors(p => { const n = { ...p }; delete n[key]; return n; });
  };

  const obAddItem = () => {
    const e = {};
    if (!obItemForm.name.trim()) e.item_name = "Obligatorio";
    if (!obItemForm.price || isNaN(parseFloat(obItemForm.price)) || parseFloat(obItemForm.price) <= 0)
      e.item_price = "Precio invalido";
    if (Object.keys(e).length) { setObErrors(e); return; }
    setObMenuItems(p => [...p, { ...obItemForm, price: parseFloat(obItemForm.price), _key: Date.now() }]);
    setObItemForm(EMPTY_ITEM);
    setObErrors({});
  };
  ...
  const obSubmit = async () => {
    setObSaving(true);
    try {
      // 1. POST /restaurants — location y contact son documentos embebidos
      const restPayload = helpers.toCreateRestaurantPayload({
        name:        obRestForm.name,
        description: obRestForm.description,
        category:    obRestForm.category,
        phone:       obRestForm.phone,
        email:       obRestForm.email,
        ownerId:     session.id,
      });
      const created = await restaurantsApi.create(restPayload);

      // 2. POST /menu-items/batch/:id — InsertMany (documentos referenciados)
      if (obMenuItems.length > 0) {
        const batchPayload = obMenuItems.map(item => ({
          name:         item.name,
        ...
        const createdItems = await menuItemsApi.createBatch(created.id, batchPayload);
      ...
      setObErrors({ submit: err.message });
    } finally {
      setObSaving(false);
    }
  };
  ...
  const handleDelete = async () => {
    try {
      await menuItemsApi.delete(deleteId);
      setItems(prev => prev.filter(i => i.id !== deleteId));
      showToast("Producto eliminado del menu");
    } catch (err) {
  ...
  const toggleAvailable = async (id) => {
    ...
    try {
      await menuItemsApi.update(id, { is_available: next });
  ...
  if (onboarding) return (
    <>
      <style>{CSS_SHARED + CSS_ONBOARD}</style>
        ...
        <div className="ob-card">
            ...
            <div className="ob-steps">
              {ONBOARD_STEPS.map((s, i) => (
                <div key={s} className="ob-step">
                  <div className={"ob-step-dot" + (i < obStep ? " done" : i === obStep ? " active" : "")}>
                    {i < obStep ? "v" : i + 1}
                  </div>
          ...
          <div className="ob-body">
            {obStep === 0 && (
              <>
                <div className="ob-field">
                  ...
                  <input type="text" placeholder="Ej. La Trattoria Italiana"
                    className={obErrors.name ? "err" : ""} value={obRestForm.name}
                    onChange={e => obHandleRestChange("name", e.target.value)} />
                  {obErrors.name && <span className="ob-err">{obErrors.name}</span>}
                </div>
                <div className="ob-field">
                  <label>Descripcion *</label>
                  <textarea rows={3} placeholder="Que tipo de comida ofreces?"
                    className={obErrors.description ? "err" : ""} value={obRestForm.description}
                    onChange={e => obHandleRestChange("description", e.target.value)} />
                  {obErrors.description && <span className="ob-err">{obErrors.description}</span>}
                </div>
                <div className="ob-field">
                  <label>Categoria principal</label>
                  <select value={obRestForm.category} onChange={e => obHandleRestChange("category", e.target.value)}>
                ...
                <div className="ob-field-row">
                  <div className="ob-field">
                    ...
                    <input type="text" placeholder="+502 5555-0000"
                      className={obErrors.phone ? "err" : ""} value={obRestForm.phone}
                      onChange={e => obHandleRestChange("phone", e.target.value)} />
                    {obErrors.phone && <span className="ob-err">{obErrors.phone}</span>}
                  </div>
                  <div className="ob-field">
                    <label>Correo de contacto *</label>
                    <input type="email" placeholder="contacto@mirestaurante.com"
                      className={obErrors.email ? "err" : ""} value={obRestForm.email}
                      onChange={e => obHandleRestChange("email", e.target.value)} />
                    {obErrors.email && <span className="ob-err">{obErrors.email}</span>}
                  </div>
                </div>
              </>
            )}
            ...
            {obStep === 1 && (
              <>
                <div className="ob-menu-builder">
                  ...
                  <div className="ob-field">
                    ...
                    <div className="ob-emoji-row">
                      {EMOJI_OPTIONS.map(e => (
                        <button key={e} className={"ob-emoji-opt" + (obItemForm.image === e ? " sel" : "")}
                          onClick={() => obHandleItemChange("image", e)}>{e}</button>
                      ))}
                    </div>
                  </div>
                  <div className="ob-field">
                    <label>Nombre del producto *</label>
                    <input type="text" placeholder="Ej. Hamburguesa Clasica"
                      className={obErrors.item_name ? "err" : ""} value={obItemForm.name}
                      onChange={e => obHandleItemChange("name", e.target.value)} />
                    {obErrors.item_name && <span className="ob-err">{obErrors.item_name}</span>}
                  </div>
                  <div className="ob-field" style={{ marginTop:10 }}>
                    <label>Descripcion</label>
                    <input type="text" placeholder="Ingredientes, preparacion..."
                      value={obItemForm.description}
                      onChange={e => obHandleItemChange("description", e.target.value)} />
                  </div>
                  <div className="ob-field-row" style={{ marginTop:10 }}>
                    <div className="ob-field">
                      <label>Precio (Q) *</label>
                      <input type="number" min="0" step="0.01" placeholder="0.00"
                        className={obErrors.item_price ? "err" : ""} value={obItemForm.price}
                        onChange={e => obHandleItemChange("price", e.target.value)} />
                      {obErrors.item_price && <span className="ob-err">{obErrors.item_price}</span>}
                    </div>
                    <div className="ob-field">
                      <label>Categoria</label>
                      <select value={obItemForm.category} onChange={e => obHandleItemChange("category", e.target.value)}>
                        {CATEGORIES.map(c => <option key={c} value={c}>{c}</option>)}
                      </select>
                    </div>
                  </div>
                  ...
                  <div className="ob-item-list">
                    {obMenuItems.map(item => (
                      <div key={item._key} className="ob-item-chip">
                        ...
                        <div className="ob-item-chip-info">
                          <div className="ob-item-chip-name">{item.name}</div>
                          <div className="ob-item-chip-meta">{item.category}{item.description ? " · " + item.description : ""}</div>
                        </div>
                ...
                {obMenuItems.length === 0 && (
                  <div className="ob-warning">Puedes continuar sin productos y agregarlos despues desde el dashboard.</div>
                )}
              </>
            )}
              ...
              <>
                <div className="ob-review-section">
                  ...
                  <div className="ob-review-row">
                    <span className="ob-review-label">Descripcion</span>
                    <span className="ob-review-val" style={{ maxWidth:"55%", fontSize:"0.82rem", color:"rgba(255,255,255,0.5)" }}>{obRestForm.description}</span>
                  </div>
                </div>
                <div className="ob-review-section">
                  ...
                  {obMenuItems.length === 0
                    ...
                    : <>
                        <div className="ob-items-badge">{obMenuItems.length} producto{obMenuItems.length !== 1 ? "s" : ""} listos para insertar</div>
                        {obMenuItems.map(item => (
          ...
          <div className="ob-footer">
            ...
            {obStep < 2
              ...
              : <button className="ob-btn-primary" onClick={obSubmit} disabled={obSaving}>
                  {obSaving && <span className="ob-spinner" />}
                  {obSaving ? "Creando..." : "Crear mi restaurante"}
                </button>
            }
          </div>
        </div>
  ...
  return (
    <>
      <style>{CSS_SHARED + CSS_DASHBOARD}</style>
          ...
          <div className="rd-topbar">
            ...
            <div className="rd-topbar-right">
              <div className="rd-search-wrap">
                ...
                <input className="rd-search" type="text" placeholder="Buscar productos..."
          ...
          <div className="rd-content">
            ...
            <div className="rd-filters">
              {["Todos", "Popular", ...availableCategories].map(c => (
                <button key={c} className={"rd-filter" + (filterCat === c ? " active" : "")} onClick={() => setFilterCat(c)}>{c}</button>
              ))}
            </div>
            ...
            <div className="rd-table-wrap">
              {filtered.length === 0 ? (
                ...
                <table className="rd-table">
                  <thead>
                    <tr><th>Producto</th><th>Categoria</th><th>Precio</th><th>Popular</th><th>Disponible</th><th>Acciones</th></tr>
                  </thead>
                  <tbody>
                    {filtered.map(item => (
                      <tr key={item.id}>
                        ...
                        <td>{item.popular ? <span className="rd-popular">Popular</span> : <span style={{ color:"rgba(255,255,255,0.2)", fontSize:"0.78rem" }}>—</span>}</td>
                        <td>
            ...
            <div className="rd-modal" onClick={e => e.stopPropagation()}>
              ...
              <div className="rd-modal-body">
                <div className="rd-field">
                  ...
                  <div className="rd-emoji-grid">
                    {EMOJI_OPTIONS.map(e => (
                      <button key={e} className={"rd-emoji-opt" + (form.image === e ? " selected" : "")}
                        onClick={() => handleFormChange("image", e)}>{e}</button>
                    ))}
                  </div>
                </div>
                <div className="rd-field">
                  <label>Nombre</label>
                  <input type="text" placeholder="Ej. Hamburguesa Clasica"
                    className={formErrors.name ? "err" : ""} value={form.name}
                    onChange={e => handleFormChange("name", e.target.value)} />
                  {formErrors.name && <span className="rd-field-error">{formErrors.name}</span>}
                </div>
                <div className="rd-field">
                  <label>Descripcion</label>
                  <textarea rows={2} placeholder="Ingredientes, preparacion..."
                    className={formErrors.description ? "err" : ""} value={form.description}
                ...
                <div className="rd-field-row">
                  <div className="rd-field">
                    <label>Precio (Q)</label>
                    <input type="number" min="0" step="0.01" placeholder="0.00"
                      className={formErrors.price ? "err" : ""} value={form.price}
                ...
                <div className="rd-field">
                  ...
                  <div className="rd-checkboxes">
                    <label className={"rd-checkbox" + (form.available ? " checked" : "")}>
                      ...
                      <div className="rd-checkbox-box">{form.available && "v"}</div>
                      Disponible
                    </label>
                    <label className={"rd-checkbox" + (form.popular ? " checked" : "")}>
                      ...
                      <div className="rd-checkbox-box">{form.popular && "v"}</div>
                      Popular
                    </label>
                  </div>
                </div>
              ...
              <div className="rd-modal-footer">
                ...
                <button className="rd-modal-save" onClick={handleSave} disabled={saving}>
                  ...
                  {saving ? "Guardando..." : editingItem ? "Guardar cambios" : "Agregar al menu"}
                </button>
              </div>
            </div>
          </div>
        ...
        {deleteId && (
          <div className="rd-overlay" onClick={() => setDeleteId(null)}>
            <div className="rd-delete-modal" onClick={e => e.stopPropagation()}>
              ...
              <p>Esto eliminara <strong style={{ color:"#fff" }}>{items.find(i => i.id === deleteId)?.name}</strong> de tu menu. Esta accion no se puede deshacer.</p>
